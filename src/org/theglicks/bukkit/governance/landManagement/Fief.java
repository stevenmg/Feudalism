...
public class Fief  extends Claim {
	...
	public Fief(Location loc){
		try {
			...
			fiefData.rs = fiefData.st.executeQuery("SELECT * FROM `governance`.`fiefs` WHERE MBRCONTAINS("
			...
			if (exists()) {
				...
				fiefOwner.rs = fiefOwner.st.executeQuery("SELECT `name` FROM `governance`.`vassals` WHERE `id` = " + ownId + ";");
				fiefOwner.rs.first();
	...
	public Fief(LandSelection select){
		try {
			fiefData = new DataStore();
			fiefData.rs = fiefData.st.executeQuery("SELECT * FROM `governance`.`fiefs` WHERE MBRIntersects(" + select.getPolygonText() + ", `region`)");
			fiefData.rs.first();
	...
	public static Fief create(Location corner, Location corner0, Player own){
		try {
			...
			fOwner.rs = fOwner.st.executeQuery("SELECT `id` FROM `governance`.`vassals` WHERE `name` = '" + own.getName() + "';");
			fOwner.rs.first();
			String ownId = fOwner.rs.getString("id");
			fData.st.execute("INSERT INTO `governance`.`fiefs` (`region`, `vassal`, `world`) VALUES (PolygonFromText('POLYGON((" + 
	...
	public void delete(){
		try {
			...
			fiefData.st.execute("DELETE FROM `governance`.`fiefs` WHERE `id`='" + id + "';");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
